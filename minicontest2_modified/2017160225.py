...
class DummyAgent(CaptureAgent):
  ...
  def chooseAction(self, gameState):
    ...
    if self.index//2==0:
      ...
      if gameState.data.agentStates[self.index].numCarrying >= 2: 
        if min([self.getMazeDistance(gameState.getAgentState(self.index).getPosition()  , enemy) for enemy in enemies]) >3:
        # 2개의 food를 먹었을 경우 점수를 get하기 위해 본인 진영으로 이동
        # 상대 agent와의 거리가 3 이하인 경우 greedy한 본인 진영으로의 이동이 아니라 offensiveevaluation에 의해 도망치는 것을 우선순위로 삼음.
          bestDist = 9999
  ...
  def offensivegetFeatures(self, gameState, action):
    ...
    for action in actionlist: # successor에서 할 수 있는 모든 action에 대하여
      ...
      # 이 100점은 offensivegetWeights에 의해 큰 -가 되어 적으로부터 도망치게 됨.
        if len(virtualsuccessor.getLegalActions(self.index))<=2:
          features['deadEnd']=100
      # 만약에 virtualsuccessor가 막다른 길이 되는 successor는
      # 큰 deadEnd penalty를 주어 선택지에서 도태시킴.
  ...
  def offensivegetWeights(self, gameState, action):
    return {'successorScore': 100, 'distanceToFood': -1, 'distanceToEnemy': -10, 'deadEnd' : -100}
